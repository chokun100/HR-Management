  ...
  return (
    <AppLayout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div className="card">
          <h3 style={{ marginBottom: 8, fontSize: 16, fontWeight: 700 }}>Employee Status</h3>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Current workforce distribution</p>
          <div style={{ display: 'flex', gap: 16 }}>
            <div style={{ flex: 1, padding: 16, background: 'rgba(16,185,129,0.08)', borderRadius: 'var(--radius-sm)', textAlign: 'center' }}>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#10b981' }}>{stats?.employeeStatus?.active || 0}</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Active</div>
            </div>
            <div style={{ flex: 1, padding: 16, background: 'rgba(245,158,11,0.08)', borderRadius: 'var(--radius-sm)', textAlign: 'center' }}>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#f59e0b' }}>{stats?.employeeStatus?.onLeave || 0}</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>On Leave</div>
            </div>
            <div style={{ flex: 1, padding: 16, background: 'rgba(239,68,68,0.08)', borderRadius: 'var(--radius-sm)', textAlign: 'center' }}>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#ef4444' }}>{stats?.employeeStatus?.resigned || 0}</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Resigned</div>
            </div>
          </div>
        </div>

        <div className="card">
          <h3 style={{ marginBottom: 8, fontSize: 16, fontWeight: 700 }}>Departments</h3>
